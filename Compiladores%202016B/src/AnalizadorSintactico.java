...
//		if ( !contieneRegreso ) error(ERR_SEM, ERR_FNC, al.obtenerLexema()); return;
	...
	void instruccion() {
		if ( al.obtenerToken().equals("identi")) {
			...
			case "interrumpe": instruccionInterrumpe(); break;
			case "continua": instruccionContinua(); break;
			case "regresa": instruccionRegresa(); break;
			default: error(ERR_SIN, "ERR_INSTRUCCION",  al.obtenerNumeroLinea(), al.obtenerLexema()); return;
			}//switch	
		}//if{}else{}
	}//
	...
	void instruccionHaz() {
		...
		while ( !(al.obtenerLexema().equals("fin") || al.obtenerLexema().equals("otro"))) {
			...
			while ( !(al.obtenerLexema().equals("caso") || al.obtenerLexema().equals("fin") ||
				...
				System.out.println(al.obtenerLexema() +"FIND ME BITCH");
				instruccion();
